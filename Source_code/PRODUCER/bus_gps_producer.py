...
# Force intentional bus bunching simulation pairs for live demo:
# Pair 1: BUS-0001 & BUS-0002 on Route R001
buses["BUS-0001"]["route_id"] = "R001"
buses["BUS-0002"]["route_id"] = "R001"
buses["BUS-0002"]["lat"] = buses["BUS-0001"]["lat"] + 0.00005
buses["BUS-0002"]["lon"] = buses["BUS-0001"]["lon"] + 0.00005

# Pair 2: BUS-0003 & BUS-0004 on Route R005
buses["BUS-0003"]["route_id"] = "R005"
buses["BUS-0004"]["route_id"] = "R005"
buses["BUS-0004"]["lat"] = buses["BUS-0003"]["lat"] + 0.00005
buses["BUS-0004"]["lon"] = buses["BUS-0003"]["lon"] + 0.00005

bus_ids = list(buses.keys())


def update_bus(bid: str, b: dict) -> None:
    """Update bus position — follower buses in bunching pairs track leader within ~10-30 metres."""
    if bid == "BUS-0002":
        leader = buses["BUS-0001"]
        b["lat"] = leader["lat"] + random.uniform(-0.00008, 0.00008)
        b["lon"] = leader["lon"] + random.uniform(-0.00008, 0.00008)
        b["speed"] = max(0.0, leader["speed"] + random.uniform(-1, 1))
        b["occupancy"] = max(0, min(b["occupancy"] + random.randint(-1, 1), 100))
    elif bid == "BUS-0004":
        leader = buses["BUS-0003"]
        b["lat"] = leader["lat"] + random.uniform(-0.00008, 0.00008)
        b["lon"] = leader["lon"] + random.uniform(-0.00008, 0.00008)
        b["speed"] = max(0.0, leader["speed"] + random.uniform(-1, 1))
        b["occupancy"] = max(0, min(b["occupancy"] + random.randint(-1, 1), 100))
